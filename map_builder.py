...
def render_map(prefix):
 ...
 for i,p in enumerate(d['places'],1):
  ...
  cards+=f'<article id="{key}" data-spot-card><img src="{prefix}assets/{p["image"]}" alt="{name} 위치 또는 풍경 안내" loading="lazy" width="{p["width"]}" height="{p["height"]}"><div><p class="eyebrow">STOP {i:02}</p><h3 tabindex="-1">{name}</h3><p>{escape(p["description"])}</p></div></article>'
