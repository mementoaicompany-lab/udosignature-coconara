...
def render(route,p):
 ...
 def navlink(r,label):
  return f'<a href="{url(r)}" data-event="content_click" data-placement="category" {"aria-current=page" if r==route else ""}>{label}</a>'
 nav=''.join(navlink(r,label) for r,label in [('','홈'),('udo-scooter/','스쿠터'),('udo-electric-car/','전기차'),('partners/','협력업체')])
 guide_routes=[('udo/','우도 여행'),('udo-ferry/','배시간'),('faq/','Q&A'),('refund/','취소·환불')]
 guide_links=''.join(navlink(r,label) for r,label in guide_routes)
 active=route in dict(guide_routes)
 nav+=f'<details class="nav-guide" data-active="{str(active).lower()}"><summary>여행·이용 안내<span class="nav-chevron" aria-hidden="true"></span></summary><div class="nav-guide-menu">{guide_links}</div></details>'
 nav+=''.join(navlink(r,label) for r,label in [('location/','오시는길'),('customer-guide/','예약 고객 안내')])
 b=C['business']
